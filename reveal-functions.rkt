...
(define (reveal-functions p)        ; Recursively replace all (Var f) references with (FunRef f arity) if the symbol f denotes a function
  ...
  (define (reveal-functions-internal e all-funs)
    (match e
      [(Var x) (cond  [(dict-has-key? all-funs x) (dict-ref all-funs x)]          ; If expression is a symbol, check if it is a function symbol, if yes, then replace it with the FunRef instance from all-funs
